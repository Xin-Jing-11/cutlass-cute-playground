"""
Persistent warp-specialized WGMMA+TMA HGEMM using CuTe DSL (TN):
    C = alpha * A^T * B + beta * C

Instead of 1 CTA per output tile, a fixed grid of CTAs (based on SM count)
loops over tiles using StaticPersistentTileScheduler.

1 producer WG (128 threads) + NCS consumer WGs = (1+NCS)*128 threads.
BM=128, BN=256, BK=64, NCS=2, QSIZE=4.
...
class HgemmPersistent:
    ...
    def __call__(
        ...
        # Create WGMMA tiled MMA with cooperative atom layout for 2 consumer WGs
        ...
        # Persistent tile scheduling: compute number of tiles and grid
        ...
        # Add batch dim (L=1) for PersistentTileSchedulerParams
    ...
    def kernel(
        ...
        # Pipeline setup: 1 DMA thread as producer, consumer warps arrive per warp
        producer_group = pipeline.CooperativeGroup(pipeline.Agent.Thread)
        num_consumer_warps = self._num_consumer_warpgroups * 4
        consumer_group = pipeline.CooperativeGroup(
            pipeline.Agent.Thread, num_consumer_warps
        )
        ...
        # TMA partitions over full global tensors
        ...
        # Get accumulator shape from a single-tile partition (no tile dims)
...
# Standalone test
# ---------------------------------------------------------------------------
def run_gemm_persistent(M=512, N=512, K=512,
                        bm=128, bn=256,
                        num_consumer_warpgroups=2,
                        num_stages=4):
    """Run the persistent warp-specialized HGEMM and verify against NumPy."""
    np.random.seed(42)
    ...
    # NumPy reference: D = alpha * A^T @ B + beta * C
    ...
    print(f"hgemm_persistent ({M}x{N}x{K}): {status}  "
