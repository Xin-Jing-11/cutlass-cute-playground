"""
Fully optimized persistent warp-specialized cluster HGEMM with TMA store
epilogue and Hilbert-curve tile scheduling (swizzle_size=4) for better L2
cache locality.

Identical to the epilogue kernel except:
  - PersistentTileSchedulerParams uses swizzle_size=4 (default)
  - Default num_stages=3

Layout convention (TN col-major, in-place):
    A(M,K):(K,1)  B(N,K):(K,1)  C(M,N):(1,M)

Tile: BM=128, BN=256, BK=64, cluster=(2,1), 2 consumer warp-groups.
"""
...
class HgemmOptimized:
    """Persistent warp-specialized cluster HGEMM with TMA store epilogue
    and Hilbert-curve tile scheduling (swizzle_size=4)."""
    ...
    def __init__(
        ...
        self._swizzle_size = swizzle_size
        ...
        num_mma_threads = self._num_mma_warp_groups * self._threads_per_wg
        self._num_mma_threads = num_mma_threads
    ...
    def __call__(
        ...
        tile_shape_mnk = (self._bm, self._bn, mma_k * 4)

        cta_layout_mnk = cute.make_layout((*self._cluster_shape_mn, 1))

        # Epilogue tile (computed outside JIT to avoid conditional assignment)
        is_cooperative = self._atom_layout_mnk == (2, 1, 1)
        epi_tile_m = min(128 if is_cooperative else 64, tile_shape_mnk[0])
        epi_tile_n = min(32, tile_shape_mnk[1])
        self._epi_tile = (epi_tile_m, epi_tile_n)
        ...
        # Persistent tile scheduler grid with swizzle
        c_shape_mn = cute.slice_(tile_shape_mnk, (None, None, 0))
        gc = cute.zipped_divide(mC, tiler=c_shape_mn)
        ...
        tile_sched_params = utils.PersistentTileSchedulerParams(
            ...
            swizzle_size=self._swizzle_size,
            raster_along_m=True,
        )
    ...
    def kernel(
        ...
        # Partition global tensors (2D: no batch dim)
        ...
        # Per-thread C partition for epilogue
        ...
        if not is_dma_warp_group:
            ...
            tRS_rD_out = cute.make_rmem_tensor(
                tRS_rD_layout.shape, self._c_dtype
            )
            ...
            while work_tile.is_valid_tile:
                ...
                # ---- Epilogue: R2S via StMatrix, S2G via TMA store ----
...
# Standalone test
# ------------------------------------------------------------------
def run_gemm_optimized(M=512, N=512, K=512,
                       bm=128, bn=256,
                       num_consumer_warpgroups=2,
                       num_stages=3,
                       cluster_m=2, cluster_n=1,
                       swizzle_size=4):
    """Run the optimized HGEMM and verify against NumPy."""
    np.random.seed(42)
    ...
    print(f"hgemm_optimized ({M}x{N}x{K}): {status}  "
