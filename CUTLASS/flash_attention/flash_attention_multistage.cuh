...
// Flash Attention "multistage" — tensor-core MMA + cp.async pipelined gmem→smem.
//
// Same algorithm as flash_attention_mma.cuh, but:
//   - K and V loaded via SM80_CP_ASYNC_CACHEGLOBAL<uint128_t> (vectorized cp.async).
//   - V cp.async lands in a natural-layout staging buffer sVr; we synchronously
//     transpose sVr → sV (MMA-ready) each j-iter. This keeps the gmem→smem path
//     async while reusing the MMA's transposed-V layout.
//   - NUM_STAGES >= 2: circular buffer over sK and sVr, with cp.async of the next
//     tile issued while the current tile's MMA + softmax executes.
//
// Requirements:
...
__global__ static void flash_attention_multistage_device(
    int seq_len,
    ...
    const cute::half_t* mask, cute::half_t* out)
{
    ...
    Tensor tXrK = s2r_thr_K.retile_D(tSrB);   // register dst (reused per stage)
    ...
    for (int tile = 0; tile < T_c; ++tile) {
        ...
        float m_ij0 = cute_fa_multi::row4_reduce_max(local_max[0]);
        float m_ij1 = cute_fa_multi::row4_reduce_max(local_max[1]);
        ...
        float l_ij0 = cute_fa_multi::row4_reduce_sum(local_sum[0]);
        float l_ij1 = cute_fa_multi::row4_reduce_sum(local_sum[1]);
...
void flash_attention_multistage(
    ...
    const cute::half_t* mask, cute::half_t* out)
{
    ...
    const size_t smem_bytes = sizeof(cute::half_t) *
        (Br * D_MODEL
         ...
         + D_MODEL * Bc                    // sV transposed (single)
         + Br * Bc);                       // sP

    dim3 grid((seq_len + Br - 1) / Br, batch_size * num_heads);
    dim3 block(NUM_WARPS * 32);

    auto kernel = flash_attention_multistage_device<Br, Bc, D_MODEL, NUM_STAGES>;
